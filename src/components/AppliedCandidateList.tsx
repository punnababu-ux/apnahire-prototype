...
export function AppliedCandidateList({ applicantCount, totalLeads = 0, dbCredits = 0, insertLeadsAfter }: Props) {
  ...
  const statusCounts = {
    all: applicantCount,
    pending: Math.round(applicantCount * 0.45),
    viewed: Math.round(applicantCount * 0.1),
    rejected: Math.round(applicantCount * 0.18),
    shortlisted: Math.round(applicantCount * 0.05),
  };
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-bold text-[#172b4d]">Applied to job</span>
        <button className="flex items-center gap-1 text-xs text-gray-600 hover:text-gray-900 transition-colors">
          ...
          <span className="font-semibold text-[#172b4d]">Most Relevant</span>
      ...
      <div className="flex gap-2 mb-3">
        {[
          { label: 'All Candidates', value: statusCounts.all,        active: true  },
          { label: 'Action Pending', value: statusCounts.pending,     active: false },
          { label: 'Viewed number',  value: statusCounts.viewed,      active: false },
          { label: 'Rejected',       value: statusCounts.rejected,    active: false },
          { label: 'Shortlisted',    value: statusCounts.shortlisted, active: false },
        ].map(s => (
          <button
            key={s.label}
            className={`flex-1 flex flex-col items-center py-3 px-2 rounded-xl border transition-colors ${
              s.active
                ? 'border-[#1f8268] bg-white'
                : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'
            }`}
          >
            <span className={`text-xl font-bold ${s.active ? 'text-[#1f8268]' : 'text-[#172b4d]'}`}>{s.value}</span>
            <span className={`text-[11px] font-medium mt-0.5 ${s.active ? 'text-[#1f8268]' : 'text-gray-400'}`}>{s.label}</span>
          </button>
        ))}
      </div>

      {/* High matches — nested container */}
        ...
        <div className="border border-purple-200 rounded-xl bg-purple-50/40 mb-3 overflow-hidden">
          <div className="px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-gray-900">High Matches</span>
            ...
            <p className="text-xs text-gray-400 mt-0.5 ml-5">Candidates matching all of your requirements. Connect with them before someone else does!</p>
          </div>
